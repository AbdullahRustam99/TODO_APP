...
BUSINESS_SERVICE_URL = os.getenv("BUSINESS_SERVICE_URL", settings.BUSINESS_SERVICE_URL)

logger.info(f"SERVICE_SECRET loaded: '{bool(SERVICE_SECRET)}'")
logger.info(f"BUSINESS_SERVICE_URL from env: {BUSINESS_SERVICE_URL}") # Log boolean status, not the secret itself.
...
# Update settings with environment variable if provided
if os.getenv("BUSINESS_SERVICE_URL"):
    BUSINESS_SERVICE_URL = os.getenv("BUSINESS_SERVICE_URL")
else:
    BUSINESS_SERVICE_URL = settings.BUSINESS_SERVICE_URL
    
logger.info(f"Final BUSINESS_SERVICE_URL: {BUSINESS_SERVICE_URL}")
...
async def add_task(
    ...
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks"
    
    logger.info(f"add_task: Making POST request to {url}")
    logger.info(f"add_task: Headers (auth redacted): Content-Type={headers.get('Content-Type')}, Has Auth={bool(headers.get('Authorization'))}")

    async with httpx.AsyncClient(trust_env=False) as client:
        try:
            response = await client.post(url, json=payload, headers=headers)
            logger.info(f"add_task: Received response status={response.status_code}")
            if response.status_code in [200, 201]:
                logger.info(f"add_task: Task added successfully")
                return {"success": True, "done": True, "message": "Task added successfully."}
            else:
                logger.error(f"add_task: Failed with status {response.status_code}: {response.text}")
                return {"success": False, "done": True, "error": f"Failed to create task: {response.status_code}", "details": response.text}
        except Exception as e:
            logger.error(f"add_task: Exception occurred: {str(e)}")
...
async def list_tasks(user_id: str, status: Literal["all", "pending", "completed"] = "all") -> dict:
    ...
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks"
    logger.info(f"list_tasks: Making GET request to {url}")
    
    async with httpx.AsyncClient(trust_env=False) as client:
        try:
            response = await client.get(url, params=params, headers=headers)
            logger.info(f"list_tasks: Received response status={response.status_code}")
...
async def complete_task(user_id: str, task_id: int) -> dict:
    ...
    if not SERVICE_SECRET:
        return {"success": False, "done": True, "error": "Internal authentication error: Service secret missing."}
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks/{task_id}/complete"
    payload = {"completed": True}
    logger.info(f"complete_task: Making PATCH request to {url}")
...
async def delete_task(user_id: str, task_id: int) -> dict:
    ...
    if not SERVICE_SECRET:
        return {"success": False, "done": True, "error": "Internal authentication error: Service secret missing."}
    headers = _get_auth_headers()
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks/{task_id}"
    logger.info(f"delete_task: Making DELETE request to {url}")
...
async def update_task(user_id: str, task_id: int, title: Optional[str] = None, description: Optional[str] = None, priority: Optional[str] = None) -> dict:
    ...
    headers = {"Content-Type": "application/json", **_get_auth_headers()}
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks/{task_id}"
    logger.info(f"update_task: Making PUT request to {url}")
...
async def set_priority(user_id: str, task_id: int, priority: str) -> dict:
    ...
    headers = {"Content-Type": "application/json", **_get_auth_headers()}
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks/{task_id}"
    logger.info(f"set_priority: Making PUT request to {url}")
...
async def list_tasks_by_priority(user_id: str, priority: str, status: Literal["all", "pending", "completed"] = "all") -> dict:
    ...
    headers = _get_auth_headers()
    url = f"{BUSINESS_SERVICE_URL}/api/{user_id}/tasks"
    logger.info(f"list_tasks_by_priority: Making GET request to {url}")
