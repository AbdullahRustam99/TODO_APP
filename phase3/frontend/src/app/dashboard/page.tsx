...
export default function DashboardPage() {
  const { user, loading, logout } = useAuth(); // Get loading from useAuth
  ...
  // Redirection on authentication state change
  useEffect(() => {
    if (!loading && !user) {
  ...
  const handleTaskSubmit = async (data: CreateTaskRequest & { id?: string; completed?: boolean }) => {
    const { id, ...taskData } = data; // Destructure id here
    try {
      if (editingTask && editingTask.id) { // Ensure id is present for update
        // Correctly call the optimistic updateTask function
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main id="main-content" className="flex-1 overflow-y-auto p-6 focus:outline-none" tabIndex={-1}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6  h-[400px] lg:h-[550px]">
              {/* AI Assistant Panel */}
              <Card className="bg-[#1C1C1C] border-gray-700 backdrop-blur-sm animate-fade-in h-full flex flex-col">
                <CardHeader className="pb-4">
